...
public class ProductDao {
	...
	public int getBottomTotalContent(Connection conn) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		int totalContent = 0;
		String sql = prop.getProperty("getBottomTotalContent");
		try {
			pstmt = conn.prepareStatement(sql);
			rset = pstmt.executeQuery();
			if(rset.next())
			...
			throw new ProductException("총 바지개수 조회 오류!", e);
		} finally {
			close(rset);
			close(pstmt);
		}
	...
	public int getTopTotalContent(Connection conn) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		int totalContent = 0;
		String sql = prop.getProperty("getTopTotalContent");
		try {
			pstmt = conn.prepareStatement(sql);
			rset = pstmt.executeQuery();
			if(rset.next())
			...
			throw new ProductException("총 상의개수 조회 오류!", e);
		} finally {
			close(rset);
			close(pstmt);
		}
	...
	public List<Product> productTypeFind(Connection conn, Map<String, Object> param) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		List<Product> list = new ArrayList<>();
		String sql = prop.getProperty("productTypeFind");
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, (String) param.get("type"));
			pstmt.setInt(2, (int) param.get("start"));
			...
			throw new ProductException("상품 조회 오류", e);
		} finally {
			close(rset);
			close(pstmt);
		}
